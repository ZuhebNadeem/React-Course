...
const cartReducer = (state, action) => {
  if (action.type === "ADD") {
    ...
    const existingCartItem = state.items[existingCartItemIndex]; //Get the exisiting card item. Will be set if the items is already added.
    let updatedItems;
  ...
  if (action.type === "REMOVE") {
    ...
    if (exisitingItem.amount === 1) {
      updateItems = state.items.filter((item) => item !== action.id);
    } else {
      const updatedItem = {
        ...exisitingItem,
